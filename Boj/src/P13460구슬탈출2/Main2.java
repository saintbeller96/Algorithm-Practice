...
public class Main2 {
	...
	static class State{
		...
		int cnt;
		String order;
		public State(char[][] map, int[][] balls, int prevDir, int cnt, String d) {
			this.map = map;
			this.balls = balls;
	...
	static State BFS(char[][] map, int[][] balls) {
		...
		for(int d = 0; d<4; d++) {
			...
			_ball[1][0] = balls[1][0];
			_ball[1][1] = balls[1][1];
			que.offer(new State(copy(map), _ball, d, 1, "" + direction[d]));
		}
		while(!que.isEmpty()) {
			...
			for(int d = 0; d<4; d++) {
				...
				que.offer(new State(copy(state.map), _ball, d, state.cnt+1, state.order + direction[d]));
			}
		}
		return null;
	}
